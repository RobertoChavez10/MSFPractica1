...
Nombre del alumno: Chavez Gonzalez Roberto
...
kP, kI, kD = 665.7796558990132,17464.7467141282,0.844695016685612
Cr = 1E-6
Re = 1/(Cr*kI)

print(f"El valor de capacitancia Cr es de {Cr} Faradios.\n")
print(f"El valor de resistencia Re es de {Re} .\n")
...
clr0 = np.array ([11, 45, 114])/ 255
clr1 = np.array ([10, 196, 224])/ 255
clr2 = np.array ([255, 155, 81])/ 255
clr3 = np.array ([0, 183, 181])/ 255
clr4 = np.array ([222, 26, 88])/ 255
clr5 = np.array ([255, 91, 91])/ 255
clr6 = np.array ([104, 90, 255])/ 255
...
plt.plot(t,PIDu1,'--',color = clr4, label = 'I(t)') # Respuesta en lazo cerrado
...
plt.plot(t,PIDu2,'--',color = clr4, label = 'I(t)') # Respuesta en lazo cerrado
plt.xlim(0,10); plt.xticks(np.arange(0,11,1))
plt.ylim(0,1.1); plt.yticks(np.arange(0,1.2,0.1))
plt.xlabel('t [s]', fontsize = 11)
plt.ylabel('Vi(t) [V]', fontsize = 11)
...
plt.plot(t,PIDu3,'--',color = clr4, label = 'I(t)') # Respuesta en lazo cerrado
plt.xlim(0,10); plt.xticks(np.arange(0,11,1))
...
plt.plot(t,PIDu4,'--',color = clr5, label = 'I(t)') # Respuesta en lazo cerrado
plt.xlim(0,10); plt.xticks(np.arange(0,11,1))
plt.ylim(0,1); plt.yticks(np.arange(-1.1,1.2,0.2))
plt.xlabel('t [s]', fontsize = 11)
plt.ylabel('Vi(t) [V]', fontsize = 11)
plt.legend(bbox_to_anchor=(0.5,-0.2),loc='center',ncol=3,fontsize=9,frameon=True)
